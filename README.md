...
│   ├── dashboard/
│   │   ├── CoinTable.tsx     # Desktop table with All / Favorites tabs
...
│   │   └── HoldingForm.tsx   # Add / edit holding form
│   ├── common/
...
│   ├── useInterval.ts        # setInterval wrapper
│   └── useDebounce.ts        # Input debounce hook
├── store/
│   ├── portfolioStore.ts     # Holdings CRUD (persisted)
...
Clearing browser storage resets all user data.
...
- Price change badges for 1h, 24h, 7d, 30d.
...
- Filtered view available on the Dashboard ("Favorites" tab).
